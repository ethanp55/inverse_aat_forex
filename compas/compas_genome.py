...
class Genome:
    def __init__(self, baseline: int, genome_length: int, features: Optional[List[int]] = None) -> None:
        ...
        data_path = './data/iaat_compas_training_data.pickle'
...
class RfGenome(Genome):
    ...
    def save_data(self) -> None:
        ...
        scaler_file = './data/compas_genetic_rf_scaler.pickle'
        rf_file = './data/compas_genetic_rf.pickle'
        features_file = './data/compas_genetic_rf_features.pickle'
        feature_importances_file = './data/compas_genetic_rf_feature_importances.pickle'
        errors_file = './data/compas_genetic_rf_errors.csv'
        ...
        with open(feature_importances_file, 'wb') as f:
            pickle.dump(list(self.feature_importances), f)
    ...
    def load_data(self) -> None:
        ...
        self.feature_importances = pickle.load(open(feature_importances_file, 'rb'))

    def feature_names(self) -> List[str]:
        feature_names = pickle.load(open('./data/iaat_compas_training_features.pickle', 'rb'))
        ...
        names = [feature_names[i] for i in range(len(feature_names)) if self.features[i] == 1]
        assert len(names) == len(self.feature_importances)
        tups = [(names[i], self.feature_importances[i]) for i in range(len(names))]
        tups.sort(key=lambda x: x[1], reverse=True)

        return [str(tup) for tup in tups]
...
class GeneticHelper(object):
    ...
    @staticmethod
    def n_features() -> int:
        training_data = np.array(pickle.load(open('./data/iaat_compas_training_data.pickle', 'rb')))
