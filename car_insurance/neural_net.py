from car_insurance.insurance_assumptions import InsuranceAssumptions, InsuranceFeatures
from car_insurance.data_cleaner import DataCleaner
import numpy as np
import pickle
...
train_iaat = np.array([InsuranceAssumptions(InsuranceFeatures(*list(x_test[i, :])),
...
with open('./data/iaat_insurance_training_data.pickle', 'wb') as f:
    pickle.dump(train_iaat, f)

aat_feature_names = InsuranceAssumptions(InsuranceFeatures(*list(x_test[0, :])), np.argmax(y_pred[0, :]))\
    .assumption_names()

with open('./data/iaat_insurance_training_features.pickle', 'wb') as f:
    pickle.dump(aat_feature_names, f)

# Save the ground truth
with open('./data/iaat_insurance_training_data_test_truth.pickle', 'wb') as f:
    pickle.dump(np.argmax(y_test, axis=1), f)

# Save the surrogate training data
with open('./data/surrogate_insurance_training_data.pickle', 'wb') as f:
