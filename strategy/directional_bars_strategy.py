...
class DirectionalBarsStrategy(Strategy):
    def __init__(self, starting_idx: int, risk_reward_ratio: float, spread_cutoff: float,
                 each_bar: bool, n_bars: int, pip_movement: int, use_pullback: bool, sl_lookback: int):
    ...
    def place_trade(self, curr_idx: int, market_data: DataFrame) -> Optional[Trade]:
        ...
        mid_highs = list(
            market_data.loc[market_data.index[curr_idx - self.sl_lookback:curr_idx], 'Mid_High'])
        mid_lows = list(
            market_data.loc[market_data.index[curr_idx - self.sl_lookback:curr_idx], 'Mid_Low'])
        mid_closes = list(
    ...
    def run_strategy(self, currency_pair: CurrencyPairs, date_range: str, learner: Optional[Learner] = None,
